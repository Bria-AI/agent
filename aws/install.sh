...
stack_name=${1:-aws-bria-agent}
bucket_name=${2:-aws-bria-agent}
...
role_arn="arn:aws:iam::$aws_account_id:role/$stack_name-role"


# copy to s3
aws s3 sync . s3://$bucket_name/ --exclude "*.py" --exclude "*.pyc"
aws cloudformation create-stack --stack-name $stack_name-role --template-body file://iam_role_stack.yml --capabilities CAPABILITY_NAMED_IAM CAPABILITY_AUTO_EXPAND --parameters ParameterKey=RoleName,ParameterValue=$stack_name-role
